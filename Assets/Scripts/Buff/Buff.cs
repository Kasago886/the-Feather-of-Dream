...
public class BurningDocumentsFeatherBuff : EquipmentFeatherBuff
{
    ...
    public override void OnUpdate()
    {
        ...
        if (attackbodyNewNum > attackbodyOriNum)
        {
            ...
            {
                if (target.attackBodyObjList[attackbodyNewNum - 1].GetComponent<AttackBody>().buffNames[i] == "灼伤")
                {
                    n++;
                }
            }
...
public class NeprizFeatherBuff : EquipmentFeatherBuff
{
    ...
    private List<string> debuffContain = new List<string> { "伤痕", "忧郁", "崩溃", "凡庸", "萎靡",
        "脆弱", "迟缓", "凝重", "中毒", "灼伤", "异常脆弱","灼伤脆弱","伤痕脆弱" ,"误解", "梦缠","心灵苦痛","释然"};
...
public class MisunderstoodWerewolfFeatherBuff : EquipmentFeatherBuff
{
    ...
    public override void OnEnter()
    {
        ...
        player.cardGenerateList.Add("无辜");
    ...
    public override void OnUpdate()
    {
        ...
        if (attackbodyNewNum > attackbodyOriNum)
        {
            ...
            for (int i = 0; i < target.attackBodyObjList[attackbodyNewNum - 1].GetComponent<AttackBody>().buffNames.Count; i++)
            {
                if (target.attackBodyObjList[attackbodyNewNum - 1].GetComponent<AttackBody>().buffNames[i] == "伤痕")
                {
                    n++;
                }
                if(target.attackBodyObjList[attackbodyNewNum - 1].GetComponent<AttackBody>().buffNames[i] == "心灵苦痛")
                {
                    n1++;
                }
            }
    ...
    public override void OnExit()
    {
        ...
        player.cardGenerateList.Remove("无辜");
...
public class EllieEquipmentFeatherBuff : EquipmentFeatherBuff
{
    ...
    public override void Init(Character target, float timer = 0, bool isPermanent = false)
    {
        ...
        //Debug.Log("max：" + feather.maxHealth+"\nnow: "+feather.health);
        this.player = target as Player;
    }
...
public class ExperimentPlayerEquipmentFeatherBuff : EquipmentFeatherBuff
{
    ...
    public override void OnEnter()
    {
        ...
        player.cardGenerateList.Add("释梦");
    }
    public override void OnUpdate()
    {
        base.OnUpdate();
    ...
    public override void OnExit()
    {
        ...
        player.cardGenerateList.Remove("释梦");
    }
}
...
public class ExperimentEffectBuff : EffectBuff
{
    Enemy Enemy;
    public override void Init(Character target, float timer = 0, bool isPermanent = false)
    {
...
public class ExperimentEffectBuff1 : EffectBuff
{
    ...
    public override void OnUpdate()
    {
        ...
        {
            timer = 0;
            target.GetComponent<Enemy>().shields.Add(new Shield()
            {
                health = 20,
                timer = 10
            });
...
public class ExperimentPlayerFeatherBuff1 : EffectBuff
{
    ...
    public override void OnEnter()
    {
        ...
        foreach (Buff buff in character.buffList)
        {
            if (buff.name == "梦源" && Num < 5)
            {
                character.RemoveBuff("梦源");
                Num++;
            }
        }
